<template>
  <div :class="platformBarStyle" ref="container" v-if="noOfPlatforms">
  ...
  computed: {
    platformBarStyle() {
      ...
      if (this.noOfPlatforms === 1) {
        className = "single-platform";
      } else {
        className = "multiple-platform";
      }
      return className;
    },
    noOfPlatforms() {
      console.log(this.moviePlatforms.length)
    ...
    moviePlatforms() {
      return Object.keys(this.stream);
    },
    finalPlatforms() {
      let finalPlat = [];
